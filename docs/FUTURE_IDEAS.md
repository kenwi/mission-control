...
- ~~**Connections & listening ports** - Summary: established count, listening sockets by port; optional "top talkers" via `ss`. Note: full socket tables may need extra privileges. *(Implemented: sortable/filterable listening ports table; established-count summary / `ss` top talkers not yet.)*~~
...
- ~~**Storage** - Device mapper / LVM hints; mount flags summary; SMART health via `smartctl` when present (slow -> cache similarly to APT-style slow metrics). *(Implemented: SMART + sysfs block detail on demand (`/api/block/{dev}`); mount options in mount detail; dm/LVM summaries not yet.)*~~

## Operational / UX

- ~~**Thresholds & highlighting** - e.g. sustained high CPU, disk use above 85%, temperature near limit: subtle borders or pill colors without building a full alerting stack. *(Implemented: thermal sensor cells vs high/crit; mount use % shading. Sustained CPU highlighting not yet.)*~~
